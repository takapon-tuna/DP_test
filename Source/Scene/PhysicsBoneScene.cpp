...
PhysicsBoneScene::PhysicsBoneScene() {
  ...
  for (int i = 0; i < _countof(bones); ++i) {
    ...
    DirectX::XMMATRIX ParentWorldTransform =
        parent != nullptr ? DirectX::XMLoadFloat4x4(&parent->worldTransform)
...
void PhysicsBoneScene::Update(float elapsedTime) {
  ...
  ImGuizmo::Manipulate(&view._11, &projection._11, ImGuizmo::TRANSLATE,
                       ImGuizmo::WORLD, &root.worldTransform._11, nullptr);
  ...
  DirectX::XMMATRIX RootWorldTransform =
      DirectX::XMLoadFloat4x4(&root.worldTransform);
  DirectX::XMMATRIX NextLocalRotationTransform =
      DirectX::XMMatrixRotationQuaternion(
          DirectX::XMLoadFloat4(&next.localRotation));
  DirectX::XMMATRIX NextLocalPositionTransform = DirectX::XMMatrixTranslation(
      next.localPosition.x, next.localPosition.y, next.localPosition.z);
  DirectX::XMMATRIX NextLocalTransform = DirectX::XMMatrixMultiply(
      NextLocalRotationTransform, NextLocalPositionTransform);
  DirectX::XMMATRIX NextWorldTransform =
      DirectX::XMMatrixMultiply(NextLocalTransform, RootWorldTransform);
