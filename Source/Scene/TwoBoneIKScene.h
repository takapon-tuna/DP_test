 ...
 private:
  struct Bone {
    ...
    void UpdateWorldTransform() {
      DirectX::XMMATRIX LocalRotationTransform =
          DirectX::XMMatrixRotationQuaternion(
              DirectX::XMLoadFloat4(&localRotation));
      DirectX::XMMATRIX LocalPositionTransform = DirectX::XMMatrixTranslation(
          localPosition.x, localPosition.y, localPosition.z);
      DirectX::XMMATRIX LocalTransform = DirectX::XMMatrixMultiply(
          LocalRotationTransform, LocalPositionTransform);
      if (parent != nullptr) {
        DirectX::XMMATRIX ParentWorldTransform =
            DirectX::XMLoadFloat4x4(&parent->worldTransform);
