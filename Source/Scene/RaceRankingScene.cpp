...
void RaceRankingScene::Update(float elapsedTime)
{
	...
	for (Car &car : cars)
	{
		...
		// TODO①:チェックポイントを通過したか判定せよ
		{
			...
			DirectX::XMVECTOR ToOldPosition = DirectX::XMVectorSubtract(OldCarPosition, NextCheckPointPosition);
			DirectX::XMVECTOR ToNowPosition = DirectX::XMVectorSubtract(NowCarPosition, NextCheckPointPosition);

			// 内積を用いて前後判定
			float dotOld = DirectX::XMVectorGetX(DirectX::XMVector3Dot(ToOldPosition, NextCheckPointDirection));
			float dotNow = DirectX::XMVectorGetX(DirectX::XMVector3Dot(ToNowPosition, NextCheckPointDirection));

			// チェックポイントを通過したか判定
			if (dotOld < 0.0f && dotNow >= 0.0f)
			{
		...
		// TODO②:チェックポイント間の進行距離を算出せよ
		{
			...
			DirectX::XMVECTOR ToNowPosition = DirectX::XMVectorSubtract(NowCarPosition, NextCheckPointPosition);
			car.progress = DirectX::XMVectorGetX(DirectX::XMVector3Dot(ToNowPosition, NextCheckPointDirection));
		}
	}
}
...
void RaceRankingScene::DrawGUI()
{
	...
	if (ImGui::Begin(u8"レース順位判定処理"))
	{
		...
		for (int i = 0; i < _countof(cars); ++i)
		{
			...
			ImGui::NextColumn();
			ImGui::Text("%.1f", car.speed);
			ImGui::NextColumn();
			ImGui::Text("%d", car.ranking);
			ImGui::NextColumn();
			ImGui::Text("%d", car.checkCount);
			ImGui::NextColumn();
			ImGui::Text("%.1f", car.progress);
			ImGui::NextColumn();
			ImGui::Separator();
		}
		ImGui::Columns(1);
