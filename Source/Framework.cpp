...
Framework::Framework(HWND hWnd) : hWnd(hWnd) {
  ...
  // IMGUI初期化
  ImGuiRenderer::Initialize(hWnd, Graphics::Instance().GetDevice(),
...
void Framework::SceneSelectGUI() {
  ...
  ImGui::SetNextWindowPos(
      ImVec2(pos.x + displaySize.x - width - 10, pos.y + 10), ImGuiCond_Once);
  ...
  if (ImGui::Begin("Scene")) {
    ...
    ChangeSceneButtonGUI<SpaceDivisionRaycastScene>(
        u8"07.空間分割レイキャスト");
    ChangeSceneButtonGUI<SortingAndFilteringScene>(
        u8"08.ソート＆フィルタリング処理");
    ChangeSceneButtonGUI<LookAtScene>(u8"09.ルックアット処理");
    ChangeSceneButtonGUI<TwoBoneIKScene>(u8"10.2本のボーンIK制御");
    ChangeSceneButtonGUI<SphereVsTriangleCollisionScene>(
        u8"11.球と三角形の衝突処理");
...
// メッセージハンドラ
LRESULT CALLBACK Framework::HandleMessage(HWND hWnd, UINT msg, WPARAM wParam,
                                          LPARAM lParam) {
